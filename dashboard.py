...
# --- 7️⃣ TEKNİK MİMARİ: BACKEND & API GÜVENLİĞİ ---
API_KEYS = [
...
SYSTEM_PROMPT = """Sen Legends Master Pro v22'sin. Şenol Kocabıyık'ın baş mimarısın. 
1) Kod üretiminde v1, v2 versiyonlama yap. 
2) Güncelleme isteklerinde mevcut kodu bozmadan geliştir. 
3) Yanıtların her zaman profesyonel ve çözüm odaklı olsun."""
...
# --- 8 MADDELİK ANAYASANIN TÜRKÇE ARAYÜZÜ ---
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="tr" class="dark">
<head>
    <meta charset="UTF-8">
    <title>Legends Master Pro | Enterprise</title>
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no, viewport-fit=cover">
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <script src="https://cdn.jsdelivr.net/npm/marked/marked.min.js"></script>
    <style>
        :root { --primary: #ffffff; --bg: #000000; --border: #333333; --card: #0A0A0A; }
        body { background: var(--bg); color: #ededed; font-family: -apple-system, sans-serif; height: 100dvh; display: flex; overflow: hidden; }
        /* 2️⃣ ANA PANEL YAPISI */
        .sidebar { width: 280px; background: var(--bg); border-right: 1px solid var(--border); transition: 0.3s; display: flex; flex-direction: column; }
        .msg-user { background: #1A1A1A; border: 1px solid var(--border); border-radius: 12px; padding: 16px; margin-left: auto; max-width: 85%; }
        .msg-ai { padding: 16px 0; max-width: 100%; border-bottom: 1px solid var(--border); display: flex; gap: 16px; }
        /* 3️⃣ SOHBET SİSTEMİ: KOD BLOKLARI */
        .markdown-body pre { background: #0A0A0A !important; border: 1px solid var(--border); border-radius: 8px; padding: 45px 16px 16px 16px; margin: 16px 0; position: relative; overflow-x: auto; }
        .code-actions { position: absolute; top: 10px; right: 10px; display: flex; gap: 8px; opacity: 0; transition: 0.2s; }
        pre:hover .code-actions { opacity: 1; }
        .code-btn { padding: 4px 10px; border-radius: 6px; font-size: 11px; font-weight: bold; border: 1px solid var(--border); color: white; cursor: pointer; }
        .btn-update { background: #2563eb; } .btn-preview { background: #16a34a; }
        /* 8️⃣ EKSTRA ÖZELLİKLER: ANIMASYONLAR */
        .modal { display: none; position: fixed; inset: 0; z-index: 9999; background: rgba(0,0,0,0.9); align-items: center; justify-content: center; padding: 20px; backdrop-filter: blur(10px); }
        .modal.active { display: flex; }
        .typing-dot { width: 4px; height: 4px; background: white; border-radius: 50%; animation: typing 1s infinite; display: inline-block; margin: 0 1px;}
        @keyframes typing { 0%, 100% { opacity: 0.3; transform:translateY(0); } 50% { opacity: 1; transform:translateY(-3px); } }
        @media (max-width: 768px) { .sidebar { position: absolute; transform: translateX(-100%); z-index: 1000; height: 100%; } .sidebar.open { transform: translateX(0); } }
    </style>
</head>
<body>

    <div id="auth-screen" class="fixed inset-0 z-[10000] bg-black flex items-center justify-center p-6">
        <div class="border border-[#333] bg-[#0A0A0A] p-10 rounded-2xl w-full max-w-sm text-center shadow-2xl">
            <i class="fas fa-cube text-5xl text-white mb-6"></i>
            <h1 class="text-2xl font-black mb-8 tracking-tighter">LEGENDS PRO</h1>
            <div class="space-y-4 mb-6">
                <input type="email" id="authEmail" class="w-full p-4 rounded-xl bg-[#111] border border-[#333] outline-none text-sm focus:border-white transition" placeholder="E-Posta Adresi">
                <input type="password" id="authPass" class="w-full p-4 rounded-xl bg-[#111] border border-[#333] outline-none text-sm focus:border-white transition" placeholder="Şifre">
            </div>
            <button id="btnLogin" class="w-full p-4 bg-white text-black rounded-xl font-bold mb-4">Giriş Yap</button>
    ...
    <aside id="sidebar" class="sidebar">
        ...
        <div id="historyList" class="flex-1 overflow-y-auto px-2 space-y-1 text-sm text-gray-400"></div>
        <div class="p-4 border-t border-[#333] flex items-center justify-between">
            <div class="flex items-center gap-2 text-sm truncate max-w-[150px]" id="userDisplay">
    ...
    <main class="flex-1 flex flex-col relative h-full">
        <header class="h-14 flex items-center justify-between px-4 border-b border-[#333] md:hidden">
            ...
            <span class="font-bold text-white flex items-center gap-2">LEGENDS</span>
        ...
        <div id="chat-container" class="flex-1 overflow-y-auto p-4 md:p-8 pb-32 space-y-6">
            <div class="flex flex-col items-center justify-center h-full text-center text-gray-500">
                ...
                <h2 class="text-2xl font-medium text-white mb-2">Merhaba Şenol, nasıl yardımcı olabilirim?</h2>
                <p class="text-sm">Legends Master Pro - v22.0</p>
            </div>
        </div>

        <div class="absolute bottom-0 w-full p-4 md:p-8 bg-gradient-to-t from-black via-black to-transparent">
            <div class="max-w-3xl mx-auto bg-[#0A0A0A] border border-[#333] rounded-2xl p-2 flex items-end gap-2 shadow-2xl focus-within:border-[#555] transition-colors">
                <button id="fileBtn" class="p-3 text-gray-400 hover:text-white transition rounded-xl"><i class="fas fa-paperclip"></i></button>
                <textarea id="userInput" class="flex-1 bg-transparent border-none text-white resize-none max-h-[200px] py-3 text-sm focus:outline-none" placeholder="Legends AI'ya bir emir ver..." rows="1"></textarea>
    ...
    <div id="settingsModal" class="modal">
        <div class="bg-[#0A0A0A] border border-[#333] p-8 rounded-2xl w-full max-w-md shadow-2xl">
            <div class="flex justify-between items-center mb-6 text-white"><h2 class="text-xl font-bold">Ayarlar</h2><i class="fas fa-times cursor-pointer text-gray-400" id="closeSettings"></i></div>
            <div class="space-y-6">
                <div>
                    <label class="flex justify-between text-xs font-bold text-gray-400 mb-2 uppercase"><span>Sıcaklık (Temp)</span><span id="tempVal">0.2</span></label>
                ...
                <div class="p-6 border border-[#333] bg-[#111] rounded-xl text-center">
                    <h3 class="font-bold text-white mb-2"><i class="fas fa-brain text-blue-500 mr-2"></i>AI Eğitme Bölümü</h3>
                    <p class="text-xs text-gray-400 mb-4">Özel verilerini analiz için yükle patron.</p>
                    <button class="w-full p-3 bg-white text-black rounded-lg text-sm font-bold">Veri Yükle</button>
                </div>
                <button id="btnLogout" class="w-full p-3 border border-red-900 text-red-500 rounded-lg text-sm font-bold hover:bg-red-900/20 transition">Oturumu Kapat</button>
            </div>
        </div>
    </div>

    <div id="previewModal" class="modal">
        <div class="bg-black border border-[#333] w-[95%] h-[90vh] rounded-2xl overflow-hidden flex flex-col shadow-2xl">
            <div class="p-4 flex justify-between items-center border-b border-[#333] bg-[#0A0A0A]">
                <div class="flex gap-2"><div class="w-3 h-3 rounded-full bg-red-500"></div><div class="w-3 h-3 rounded-full bg-yellow-500"></div><div class="w-3 h-3 rounded-full bg-green-500"></div><span class="ml-4 text-xs font-bold text-gray-400">CANLI ÖNİZLEME MOTORU</span></div>
    ...
    <script type="module">
        ...
        renderer.code = function(code, language) {
            ...
            return `<div class="relative">
                <div class="code-actions">
                    <button onclick="window.updateCode(\`${escapedCode}\`)" class="code-btn btn-update">Güncelle v2</button>
                    <button onclick="window.copyCode(\`${escapedCode}\`, this)" class="code-btn bg-[#1A1A1A]">Kopyala</button>
                    <button onclick="window.previewCode(\`${escapedCode}\`)" class="code-btn btn-preview">Önizle</button>
                </div>
        ...
        window.copyCode = (code, btn) => { navigator.clipboard.writeText(code); btn.innerHTML = 'Bitti!'; setTimeout(() => btn.innerHTML = 'Kopyala', 2000); };
        ...
        onAuthStateChanged(auth, (u) => {
            if(u){
                ...
                document.getElementById('userDisplay').innerHTML = `<div class="w-7 h-7 rounded-full bg-white text-black flex items-center justify-center font-bold text-xs">${u.email[0].toUpperCase()}</div><span class="truncate">${u.email.split('@')[0]}</span>`;
        ...
        function syncHistory() {
            ...
            onSnapshot(q, (snap) => {
                ...
                snap.forEach(d => {
                    ...
                    div.innerHTML = `<span class="truncate pr-2">${d.data().title || 'Yeni Sohbet'}</span><i class="fas fa-trash-alt opacity-0 group-hover:opacity-50 hover:text-red-500" onclick="event.stopPropagation(); deleteChat('${d.id}')"></i>`;
...
async function startNewChat() {
            ...
            document.getElementById('chat-container').innerHTML = '<div class="flex flex-col items-center justify-center h-full text-center text-gray-500"><i class="fas fa-cube text-5xl mb-6 text-white opacity-20"></i><h2 class="text-2xl font-medium text-white mb-2">Nasıl yardımcı olabilirim Şenol?</h2></div>';
        ...
        window.deleteChat = async (id) => { if(confirm("Silmek istiyor musun?")) { await deleteDoc(doc(db, `users/${currentUser.uid}/chats`, id)); startNewChat(); } };
        ...
        async function sendMessage() {
            ...
            } catch(e) { document.getElementById(loadId).innerHTML = "❌ Bağlantı hatası."; }
        }
        ...
        function addMsgUI(t, r) {
            ...
            else d.innerHTML = `<div class="w-8 h-8 rounded-full border border-[#333] flex items-center justify-center flex-shrink-0 bg-white"><i class="fas fa-cube text-xs text-black"></i></div><div class="flex-1 markdown-body text-[#ededed] leading-relaxed break-words overflow-hidden">${marked.parse(t)}</div>`;
        ...
        document.getElementById('fileBtn').onclick = () => alert('Dosya yükleme sistemi yakında aktif olacak patron!');
    </script>
</body>
</html>
"""
