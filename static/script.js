...
let cUser = null, cChatId = null, history = [], config = { temperature: 0.4, systemPrompt: "" };
...
renderer.code = function(code, lang) {
    ...
    <div class="code-toolbar">
        <button onclick="window.uC('${b64}')" class="code-btn btn-v2"><i class="fas fa-level-up-alt"></i> v2 Yap</button>
...
window.uC = (b) => { document.getElementById('userInput').value = `Mevcut kodu çok daha üstün bir seviyeye taşıyarak v2 sürümünü yap:\\n\\n\`\`\`\\n${getRaw(b)}\`\`\``; document.getElementById('userInput').focus(); };
window.cC = (b, btn) => { navigator.clipboard.writeText(getRaw(b)); btn.innerHTML = 'Alındı'; setTimeout(()=>btn.innerHTML='<i class="fas fa-copy"></i> Kopyala', 2000); toast('Kod kopyalandı.'); };
window.pC = (b) => { document.getElementById('previewModal').classList.add('active'); document.getElementById('pFrame').srcdoc = getRaw(b); };
window.dwn = (b) => { const blob = new Blob([getRaw(b)], {type: "text/html"}); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = "Legends_Proje.html"; a.click(); toast('Dosya İndirildi.'); };
...
document.getElementById('saveSet').onclick = async () => { config.systemPrompt = document.getElementById('sysPrompt').value; config.temperature = parseFloat(document.getElementById('tRange').value); await setDoc(doc(db, `users/${cUser.uid}/settings`, 'cfg'), config); toast('Süper Zeka Kaydedildi.'); document.getElementById('settingsModal').classList.remove('active'); };
...
async function sM() {
    ...
    const d = document.createElement('div'); d.className = 'msg-ai'; d.innerHTML = `<div class="markdown-body"><i class="fas fa-circle-notch fa-spin text-blue-500"></i> Sistem analiz ediyor...</div>`;
    ...
    try {
        ...
        // HATA SİLME KORUMASI EKLENDİ!
        cDiv.innerHTML += `<br><br><span style="color:#ef4444; font-weight:bold; padding:10px; border:1px solid #ef4444; border-radius:8px; display:inline-block;"><i class="fas fa-plug"></i> Sinyal koptu ancak yazılan kodlar korundu patron. Kaldığı yerden devam etmesini isteyebilirsin.</span>`; 
    }
}
